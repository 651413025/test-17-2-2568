...
export default function ChatUI() {
  const [messages, setMessages] = useState([
    { text: "สวัสดี! วันนี้ฉันจะช่วยคุณได้อย่างไร?", sender: "bot" },
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      let endpoint;https://github.com/651413025/test-17-2-2568
      if (mode === "tokenize") {
        endpoint = "tokenize";
      } else if (mode === "textqa") {
        endpoint = "textqa";
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 max-w-4xl mx-auto w-full">
      <div className="p-4 flex justify-center">
        ...
        <Button onClick={() => setMode("th2en")} className={`mr-2 ${mode === "th2en" ? "bg-blue-500 text-white" : "bg-gray-300"}`}>แปลเป็นอังกฤษ</Button>
      </div>
